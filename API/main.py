...
#    OUT =  Unstructure2Struct(TEXT_FILE) 
    ...
    OUT = OUT[0:26]
    OUT['Editable Extracted_Values'] = OUT["Extracted_Values"]
...
@app.route("/chartjs_New", methods=['GET', 'POST'])
def chartjs():
    try:
...
#                        return jsonify(name=name,imgfile=imgfile,dept=dept)
        ...
        return render_template("chartjs_New.html",result='No data available in text', pdf_location=pdf_location)
